...
function instializeInSetup(character){
	...
	character.scale=1;
  character.setCollider("rectangle",0,0,28,38)
...
function standOnObjs(obj1,obj2){
	...
	if(obj1_Right>=obj2_Left&&obj1_Left<=obj2_Right && obj1_Down<=obj2_Up+7 && obj1_Down>=obj2_Up-7){
		// println("YES");
		obj1.velocity.y = 0;
		obj1.position.y=obj2_Up-(obj1.height/2);
...
function stateOfEnemy(enemy){
  if (enemy.live==false||enemy.position.y>gameConfig.screenY+50){
    ...
    if(enemy.fly){
      enemy.position.y=random(gameConfig.screenY*0.1,gameConfig.screenY*0.3);
    } else{
...
function moveEnvironment(character){
  var environmentScrollingSpeed=gameConfig.moveSpeed*gameConfig.velocidadfondo; 
...
function debugging(character){
  ...
  enemyAvispas.forEach(function(element){ outline(element); });
